...
abstract final class AppLinkConfig {
  ...
  static const String _deployedApiUrl = 'https://api.studio-3.co';
  ...
  // These links are built against the API origin (api.studio-3.co), which is why it is
  // also listed as its own host in ios/Runner/Runner.entitlements and the Android intent
  // filters, alongside studio-3.co — Apple and Android only honour a Universal/App Link on
  // a domain the app is associated with, and api.studio-3.co is a different host from
  // studio-3.co even though it shares the same registrable domain. The backend serves
  // /.well-known/apple-app-site-association and /.well-known/assetlinks.json itself
  // (src/modules/share/well_known_routes.py), so this is option 2 from the note this
  // replaced: the API origin is in the associated domains, and it answers for itself.
